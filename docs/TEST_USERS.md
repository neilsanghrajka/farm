...
This file tracks reusable production QA identities for Farm, AgentMail, and X testing. These accounts are test-only, must not contain real user data, and must never be used for production customer activity.
...
These slots are reserved for Auto Engage production smoke tests. Update the table after each account is actually created and verified.

Provisioning status: these are reserved identity slots until the AgentMail inbox, Farm user, and X account are all created and linked. Do not treat a row as usable for production Auto Engage smoke tests until Farm role/status is active and Linked-X status is linked.

| Slot | AgentMail email | Farm email | X handle | Farm role/status | Linked-X status | Created date |
| --- | --- | --- | --- | --- | --- | --- |
| 1 | `farm-auto-qa-01@agentmail.to` | `farm-auto-qa-01@agentmail.to` | TBD by X | Farm user created; admin in `Farm Auto QA` | X signup/login created, then blocked by X; not linked | 2026-05-07 (AgentMail + Farm) |
| 2 | `farm-auto-qa-02@agentmail.to` | `farm-auto-qa-02@agentmail.to` | TBD by X | Farm user created; member in `Farm Auto QA` | X not created; do not retry until X blocker strategy is decided | 2026-05-07 (AgentMail + Farm) |
| 3 | `farm-auto-qa-03@agentmail.to` | `farm-auto-qa-03@agentmail.to` | TBD by X | Farm user created; member in `Farm Auto QA` | X not created; do not retry until X blocker strategy is decided | 2026-05-07 (AgentMail + Farm) |
...
TEST_USER_1_X_USERNAME=farm_auto_qa_01
TEST_USER_1_X_PASSWORD=

TEST_USER_2_EMAIL=farm-auto-qa-02@agentmail.to
TEST_USER_2_FARM_PASSWORD=
TEST_USER_2_X_USERNAME=farm_auto_qa_02
TEST_USER_2_X_PASSWORD=

TEST_USER_3_EMAIL=farm-auto-qa-03@agentmail.to
TEST_USER_3_FARM_PASSWORD=
TEST_USER_3_X_USERNAME=farm_auto_qa_03
TEST_USER_3_X_PASSWORD=
```

Do not store AgentMail magic links, X recovery codes, one-time verification codes, OAuth access tokens, refresh tokens, or Convex account secrets in `.env.local`. Prefer regenerating one-time values.
...
4. Leave the inboxes active for future X and Farm verification emails.

If AgentMail API access is missing or an inbox cannot receive mail, stop and fix AgentMail before creating Farm or X accounts. Farm/X verification depends on receiving email codes reliably.
...
Do not automate new X account creation for Farm QA. The AgentMail-based X signup strategy failed because X blocked the newly created slot-1 account after signup/login. Future Auto Engage production smoke tests should use manually maintained, aged X test accounts that the user has already unblocked and can sign into normally.
...
3. Store only the stable X test username/password in `.env.local` if the user explicitly wants local reuse.
...
- Do not create slots 2 and 3 on X in the same browser/session/IP until the team decides a safer X QA strategy. More rapid fresh-account creation is likely to increase blocks.
- Prefer one of these next strategies:
  ...
  - Create X test accounts manually with normal human pacing, then only use Farm to link them through official OAuth.
...
2. Sign into X as the matching X account in the same browser session used for OAuth.
...
Use the three test-user triples for production Auto Engage smoke tests.
...
- Never delete AgentMail inboxes that are still referenced by active Farm or X accounts unless the whole test identity is being retired.

## Learnings

- AgentMail inboxes should be created before Farm or X signup so verification emails can be received without switching plans mid-flow.
...
- X signup may require human challenges. Treat those as stop points, not automation problems.
- If X blocks a newly created QA account, stop all further X account creation attempts for that session and record the blocker before retrying.
...
- The tracked source of truth is this document for account inventory, and ignored `.env.local` for credentials.
